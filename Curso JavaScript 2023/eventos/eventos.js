...
// const textToClick = () => console.log(buttonElement.textContent);
...
const getFinalMessage = (event) => {
  if (event.altKey && event.key !== 'Alt') {
    paragraph.textContent = `Alt + ${event.key}`;
  } else if (event.ctrlKey && event.key !== 'Control') {
    paragraph.textContent = `Ctrl + ${event.key}"`;
  } else if (event.shiftKey && event.key !== 'Shift') {
    paragraph.textContent = `Shift + ${event.key}"`;
  } else {
    paragraph.textContent = `${event.key}`;
  }
  return paragraph.textContent;
};

const finalMsg = (event) => {
  const final = getFinalMessage(event);
  paragraph.textContent = `Has pulsado: ${final}`;
};

window.addEventListener('keydown', finalMsg);
...
const showDate = (event) => {
  dateElement.previousElementSibling.textContent = `Has seleccionado ${dateElement.value}`;
};
...
// const nextFunction = () => {
//   if (index >= 0 && index <= computersList.length - 1) {
//     textElement.textContent = computersList[index++];
//   } else if (index === computersList.length) {
//     index = 0;
//     textElement.textContent = computersList[index++];
//   }
// };
...
//     textElement.textContent = computersList[index--];
//   } else if (index < 0) {
//     index === computersList.length - 1;
//     textElement.textContent = computersList[index--];
//   }
// };

// nextButtonElement.addEventListener('click', nextFunction);
// preButtonElement.addEventListener('click', prevFunction);
...
//   if (usernameElement.value.length > 3 && passwordElement.value.length > 4) {
//     activateSubmit();
//   }
// });
